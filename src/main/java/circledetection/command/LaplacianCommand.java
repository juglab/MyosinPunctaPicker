...
public class LaplacianCommand< T extends RealType< T > > {
	...
	public LaplacianCommand( Img< T > image, double sigma, int axis, double samplingFactor, OpService ops ) {
		this( image, sigma, axis, samplingFactor, image, ops );
	}

	public LaplacianCommand( Img< T > image, double sigma, int axis, double samplingFactor, Interval outputInterval, OpService ops ) {
		this.image = image;
		this.sigma = sigma;
		this.axis = axis;
    ...
    private RandomAccessibleInterval<FloatType> secondDerivative(final RandomAccessibleInterval<T> input, final double sigma, final int d) {
		...
		final Img< FloatType > output = ops.create().img( outputInterval, new FloatType() );


		double scaledSigma = ( d == axis ) ? sigma / samplingFactor : sigma;
		final Kernel1D second_derivative = DerivedNormalDistribution.derivedGaussKernel( scaledSigma, 2 );
